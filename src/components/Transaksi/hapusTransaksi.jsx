...
const HapusTransaksi = ({idTransaksi,namaPelanggan,callback}) => {
    ...
    const handleHapus = async (e) => {
        ...
        try {
            const result = await deleteData(`/penjualan/hapus/${idTransaksi}`);
            if (result === 200) {
                  onClose();
                  callback(true);
                  toast({
                      title: 'Berhasil.',
                      description: "Hapus Transaksi Berhasil",
                      status: 'success',
                      duration: 3000,
                      position: 'top',
                      isClosable: true,
  ...
  return (
    <div>
        ...
        <ModalContent p='5'>
        <ModalBody>
            <Text textAlign='center' fontWeight='bold'>Apakah anda yakin ingin menghapus transaksi ?</Text>
            <Text textAlign='center' fontWeight='bold'>atas nama :</Text>
